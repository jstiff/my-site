FUTURE_DATA = [
  "M",
  "y",
  " ",
  "g",
  "o",
  "a",
  "l",
  " ",
  "i",
  "n",
  " ",
  "t",
  "h",
  "e",
  " ",
  "f",
  "u",
  "t",
  "u",
  "r",
  "e",
  " ",
  "i",
  "s",
  " ",
  "t",
  "o",
  " ",
  "l",
  "e",
  "a",
  "r",
  "n",
  " ",
  "s",
  "y",
  "s",
  "t",
  "e",
  "m",
  "s",
  " ",
  "p",
  "r",
  "o",
  "g",
  "r",
  "a",
  "m",
  "m",
  "i",
  "n",
  "g",
  " ",
  "i",
  "n",
  " ",
  "C",
  " ",
  "a",
  "n",
  "d",
  " ",
  "R",
  "u",
  "s",
  "t",
  ".",
  ".",
  ".",
  "\n",
  "I",
  " ",
  "a",
  "l",
  "s",
  "o",
  " ",
  "h",
  "o",
  "p",
  "e",
  " ",
  "t",
  "o",
  " ",
  "o",
  "n",
  "e",
  " ",
  "d",
  "a",
  "y",
  " ",
  "c",
  "o",
  "n",
  "t",
  "r",
  "i",
  "b",
  "u",
  "t",
  "e",
  " ",
  "t",
  "o",
  " ",
  "o",
  "p",
  "e",
  "n",
  " ",
  "s",
  "o",
  "u",
  "r",
  "c",
  "e",
  " ",
  "s",
  "o",
  "f",
  "t",
  "w",
  "a",
  "r",
  "e",
  ".",
  ".",
  ".",
  ".",
  "\n",
  "T",
  "h",
  "a",
  "n",
  "k",
  " ",
  "y",
  "o",
  "u",
  " ",
  "f",
  "o",
  "r",
  " ",
  "v",
  "i",
  "s",
  "i",
  "t",
  "i",
  "n",
  "g",
  " ",
  "m",
  "y",
  " ",
  "w",
  "e",
  "b",
  "s",
  "i",
  "t",
  "e",
  "!",
];

PRESENT_DATA = [
  "I",
  " ",
  "a",
  "m",
  " ",
  "c",
  "u",
  "r",
  "r",
  "e",
  "n",
  "t",
  "l",
  "y",
  " ",
  "w",
  "o",
  "r",
  "k",
  "i",
  "n",
  "g",
  " ",
  "o",
  "n",
  " ",
  "a",
  " ",
  "g",
  "i",
  "t",
  "h",
  "u",
  "b",
  " ",
  "A",
  "p",
  "i",
  " ",
  "a",
  "p",
  "p",
  "l",
  "i",
  "c",
  "a",
  "t",
  "i",
  "o",
  "n",
  " ",
  "t",
  "h",
  "a",
  "t",
  " ",
  "a",
  "l",
  "l",
  "o",
  "w",
  "s",
  " ",
  "u",
  "s",
  "e",
  "r",
  "s",
  " ",
  "t",
  "o",
  " ",
  "p",
  "r",
  "a",
  "c",
  "t",
  "i",
  "c",
  "e",
  " ",
  "t",
  "y",
  "p",
  "i",
  "n",
  "g",
  " ",
  "o",
  "v",
  "e",
  "r",
  " ",
  "c",
  "o",
  "d",
  "e",
  ".",
  ".",
  ".",
  ".",
  "\n",
  "Y",
  "o",
  "u",
  " ",
  "c",
  "a",
  "n",
  " ",
  "c",
  "h",
  "e",
  "c",
  "k",
  " ",
  "i",
  "t",
  " ",
  "o",
  "u",
  "t",
  " ",
  "b",
  "y",
  " ",
  "c",
  "l",
  "i",
  "c",
  "k",
  "i",
  "n",
  "g",
  " ",
  "t",
  "h",
  "i",
  "s",
  " ",
  "l",
  "i",
  "n",
  "k",
  " ",
  "%",
  " ",
];

let PAST_DATA = [
  " ",
  "I",
  " ",
  "h",
  "a",
  "v",
  "e",
  " ",
  "b",
  "u",
  "i",
  "l",
  "t",
  " ",
  "m",
  "a",
  "n",
  "y",
  " ",
  "t",
  "h",
  "i",
  "n",
  "g",
  "s",
  " ",
  "i",
  "n",
  " ",
  "t",
  "h",
  "e",
  " ",
  "p",
  "a",
  "s",
  "t",
  ".",
  ".",
  ".",
  "\n",
  " ",
  "T",
  "i",
  "m",
  "b",
  "e",
  "r",
  " ",
  "F",
  "r",
  "a",
  "m",
  "e",
  " ",
  "h",
  "o",
  "m",
  "e",
  "s",
  " ",
  "f",
  "r",
  "o",
  "m",
  " ",
  "l",
  "o",
  "g",
  "s",
  " ",
  "I",
  " ",
  "c",
  "u",
  "t",
  " ",
  "d",
  "o",
  "w",
  "n",
  " ",
  "m",
  "y",
  "s",
  "e",
  "l",
  "f",
  ".",
  ".",
  ".",
  "\n",
  " ",
  "J",
  "a",
  "p",
  "a",
  "n",
  "e",
  "s",
  "e",
  " ",
  "w",
  "o",
  "o",
  "d",
  " ",
  "f",
  "i",
  "r",
  "e",
  "d",
  " ",
  "C",
  "e",
  "r",
  "a",
  "m",
  "i",
  "c",
  " ",
  "k",
  "i",
  "l",
  "n",
  "s",
  " ",
  "f",
  "o",
  "r",
  " ",
  "m",
  "a",
  "k",
  "i",
  "n",
  "g",
  " ",
  "p",
  "o",
  "t",
  "t",
  "e",
  "r",
  "y",
  ".",
  ".",
  ".",
  "\n",
  " ",
  "A",
  "n",
  "d",
  " ",
  "a",
  " ",
  "c",
  "o",
  "m",
  "p",
  "l",
  "e",
  "t",
  "e",
  " ",
  "m",
  "o",
  "t",
  "o",
  "r",
  "c",
  "y",
  "c",
  "l",
  "e",
  " ",
  "f",
  "r",
  "o",
  "m",
  " ",
  "s",
  "c",
  "r",
  "a",
  "t",
  "c",
  "h",
  ".",
  ".",
  ".",
];

let area = document.getElementById("textContainer");
let presentFuture = () => {
  area.innerHTML = "";
  for (let i = 0; i < FUTURE_DATA.length; i++) {
    (function (i) {
      setTimeout(function () {
        area.innerHTML += FUTURE_DATA[i];
        console.log(FUTURE_DATA[i]);
      }, 50 * i);
    })(i);
  }
};

let presentPast = () => {
  area.innerHTML = "";
  for (let i = 0; i < PAST_DATA.length; i++) {
    (function (i) {
      setTimeout(function () {
        area.innerHTML += PAST_DATA[i];
        console.log(PAST_DATA[i]);
      }, 50 * i);
    })(i);
  }
};

let presentPresent = () => {
  area.innerHTML = "";
  let gitHub = `https://github.com/jstiff/Imitate`;

  for (let i = 0; i < PRESENT_DATA.length; i++) {
    if (PRESENT_DATA[i] === "%") {
      (function (i) {
        // let textNode2 = document.createTextNode(gitHub);
        setTimeout(function () {
          let a = document.createElement("a");
          let textNode2 = document.createTextNode(gitHub);
          a.appendChild(textNode2);
          a.href = gitHub;
          area.appendChild(a);
          //   node.appendChild(textNode2);

          console.log(PRESENT_DATA[i]);
        }, 50 * i);
      })(i);
    } else {
      (function (i) {
        setTimeout(function () {
          let span = document.createElement("SPAN");
          let textNode = document.createTextNode(PRESENT_DATA[i]);
          span.appendChild(textNode);
          area.appendChild(span);
          console.log(PRESENT_DATA[i]);
        }, 50 * i);
      })(i);
    }
  }
};

document.getElementById("past").addEventListener("click", presentPast);
document.getElementById("present").addEventListener("click", presentPresent);
document.getElementById("future").addEventListener("click", presentFuture);
